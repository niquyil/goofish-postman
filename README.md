...
而不是数字账号，方便和网页上的账号卡片对上），下面第一段是**代码框形式的明细**，再下面是消息正文。
每个账号固定一种标题配色（按账号 id 哈希），扫一眼颜色就知道是哪个号收到的：

```
标题      网课学习私人助理 → xy773249480508(小号)
明细      ┌────────────────────────────────────────┐
（代码框）│ 时间：09-11 23:55:32                     │
          │ 商品：正品包邮 上海迪士尼olu星梦旋律系列…   │
          └────────────────────────────────────────┘
正文      在吗，这个还在吗？
```

> 标题已经承担了「谁发给谁」，所以正文不再套代码框 —— 代码框留给明细，两者一眼分得开；
...
> - **14 提示条不推飞书**（`SILENT_CONTENT_TYPES`）：这类提示每个会话都会反复出现，
>   实测某个账号 248 条历史里有 43 条，属于噪音；网页消息流仍然记录，随时能回查；
> - **图片不能直接内嵌在飞书卡片里**：飞书只接受 `image_key`（要用
>   `POST /open-apis/im/v1/images` 上传拿，接口需要自建应用的 `app_id`/`app_secret`），
>   把外部 http 地址写进 `![文字](地址)` 会被直接拒收
>   （实测 `ErrCode 200570 invalid image keys`）。所以现在给的是可点开的原图地址。

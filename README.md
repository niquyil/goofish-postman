...
1. 在「飞书推送」里填**应用 ID / 应用密钥**，点「获取群列表」挑一个**目标群**保存 ——
   ...
   群列表**只在点按钮时拉一次并缓存在本地**（`accounts.json` 的 `notify.chats`，页面下方会显示缓存时间）：
   重开页面、重启程序都直接用缓存，不会再打飞书接口；机器人新加了群、或换了应用，再点一次刷新即可。
2. 添加账号有两种方式：
   - **📱 扫码添加（推荐）**：点按钮后用手机闲鱼 App 扫二维码并在手机上确认，登录成功后账号会自动建好并开始监听；
   - **粘贴 Cookie**：从浏览器开发者工具复制登录后的完整 Cookie。
3. 每个账号卡片上有四个操作（鼠标悬停有说明）：
   - **启停开关**：开 = 开始监听，关 = 停掉监听；
   - **重连**：停掉当前长连接，立刻用**现有 Cookie** 重新连一次（不改配置）——
     连接卡住、或正在等重试倒计时（`x 秒后重试`）时用它催一下，状态会走「已停止 → 连接中 → 监听中」；
   - **改 Cookie**：粘贴新的登录 Cookie，保存后自动重连（Cookie 失效时用这个，不是「重连」）；
   - **删除**：把账号从配置里删掉并停止监听。
4. **登录态是自己续期的**（不用管）：mtop token 的有效期约 2 小时，长连接每 10 分钟续一次，
   续期后**把轮换过的 cookie 写回 `accounts.json`** —— 所以断线重连、重启程序用的都是最新登录态，
   不会拿几天前那份早就过期的 cookie 去换 token。账号卡片上会显示「登录态剩余 x 小时 x 分」，
   剩余不足 30 分钟（说明续期没成功）时事件流里会有一条 warn 提醒。
5. Cookie 真的过期时（账号卡片状态变成「异常」，并写明「登录态已失效…」），点「改 Cookie」粘贴新的，
   或重新扫码登录即可；服务会按退避自动重连，**换上新 Cookie 就会自愈，不用重启程序**。
   长期挂机时建议偶尔看一眼账号卡片上的登录态剩余时间。
...
明细      时间 09-11 23:55:32
          商品 正品包邮 上海迪士尼olu星梦旋律系列…
          ────────────────────────────────────────────
正文      在吗，这个还在吗？
```
...
> 「消息属性」和「正文」分开；每个属性单独一行，字段多了也不会挤成一长条。
>
...
|-------------|------|------------------------------------------------------------------|
| 1           | 文本   | 原文                                                               |
| 2           | 图片   | `[图片]` + 每张图的地址；配了应用则**直接内嵌显示**                                  |
| 3           | 语音   | `[语音]` + 时长 + 地址；能转 OPUS 时**卡片下面补一条可播放的语音消息**                     |
| 4           | 视频   | `[视频]` + 播放地址；配了应用则**在卡片里内嵌播放器**                                 |
...
| 类型 | 飞书侧怎么放                                  | 失败时              |
|----|-----------------------------------------|------------------|
| 图片 | 上传换 `image_key`，内嵌进卡片正文                  | 退回可点链接           |
| 视频 | 上传换 `file_key`（`file_type=mp4`，≤30MB），封面换 `img_key`，用卡片 2.0 的 `video` 组件内嵌 | 退回可点链接 |
| 语音 | 卡片放不下音频，先发卡片再补一条 `audio` 消息（`file_type=opus`） | 退回可点链接 |

> - 视频组件要求卡片关掉转发（`enable_forward: false`），否则整张卡片发不出去，代码里已处理；
>   该组件需要飞书客户端 V7.56+，低版本会显示一句降级提示；
> - **视频时长从 mp4 文件里读**：闲鱼报文里的 `video.duration` 实测一直是 0，直接用会让飞书
>   卡片显示 00:00；现在会解析 mp4 的 `moov/mvhd`（自己读 box，不引额外依赖），
>   实测那条真实视频读出 19412 ms，与播放器显示的 19 秒一致；
> - **语音飞书只收 OPUS**：闲鱼语音一般是 AMR/其他格式，所以本机装了 `ffmpeg` 时会自动转一道
>   （16k 单声道）；没装就保留链接（日志里会说明）。要开这个能力：装好 ffmpeg 并在 PATH 里即可；
> - 报文里的时长换算是 `to_milliseconds`：单位没有权威文档，数值小于 1000 时按秒处理。
...
- 图片 / 视频 / 语音都是「先下载（这一步仍用 httpx）、再上传换 key」：图片换 `image_key`
  内嵌进卡片，视频换 `file_key` 用 `video` 组件内嵌，语音换 `file_key` 后补发一条
  `audio` 消息；**同一个地址只上传一次**（按地址缓存）；
- 内嵌成功时正文里对应的标注与地址行会被去掉（语音保留 `[语音]` 标注，好和下面的语音对上）；
  上传/下载失败时自动退回可点开的链接，不会漏消息。

三个踩过的坑，供参考：
- 下载闲鱼图片/视频要带 `Referer: https://www.goofish.com/`，不带的话部分地址会返回 420
  （实测同一张图裸请求 420、加 Referer 后 200）；SDK 不管这一步，所以下载仍走 httpx；
- 列群接口要 `im:chat:readonly` 权限，且机器人必须已经在群里，否则拿不到群列表；
- 卡片里放视频必须显式关掉转发（`enable_forward: false`），否则整张卡片发不出去。

#### 在飞书里回复卡片 → 用对应账号发到闲鱼

在飞书群里**引用回复**机器人发的那张卡片（顺手 @ 一下机器人），回复内容就会用**对应的账号**
发到**对应的闲鱼会话**；成功/失败都会以「回复你那条消息」的形式告诉你。

**第一步：飞书开放平台的控制台必须配好这三项**（少任何一项都收不到事件，服务端不会有任何日志）：

| 位置 | 要做什么 |
|------|---------|
| 事件订阅 → 订阅方式 | 选 **长连接**（不需要公网回调地址；用 webhook 方式就得有公网 HTTPS，本项目不支持） |
| 事件订阅 → 事件 | 添加 **接收消息 v2.0**（`im.message.receive_v1`）—— 只选订阅方式不加事件，一样收不到 |
| 权限管理 | 加一个能收群消息的权限（下面二选一） |
| 版本管理 | **创建版本并发布** —— 事件与权限的改动都要发版才生效 |

收群消息的权限二选一：

- `im:message.group_at_msg:readonly`（推荐：只收 @ 机器人的消息，**要求每条回复都 @ 机器人**）
- `im:message.group_msg`（敏感权限，需申请开通：能收群里所有消息，回复时不必 @）

> 这段是真踩过的坑：一开始只配了权限、没在「事件订阅」里把订阅方式设成长连接并添加接收消息事件，
> 结果飞书完全不推事件，服务端一条日志都没有，看起来就像功能没生效。

用法与限制：

- **必须 @ 一下机器人**（同时「引用回复」那张卡片）：只申请了 `im:message.group_at_msg*` 时
  飞书只推送 @ 机器人的消息，不 @ 根本不会送事件过来；
- **只有引用的是「机器人转发闲鱼消息的那张卡片」才会处理并反馈**：
  - 引用的就是那张卡片 → 发到闲鱼，并在飞书里回一句
    `已通过<发送方昵称>（<发送方账号>）向<对方昵称>回复`（失败则在这句后面接原因）；
  - 引用的是别人的消息、或者只 @ 机器人没引用 → **不在群里回话**（免得刷屏），
    只在控制台日志与网页「事件日志」里记一笔 `未转发飞书消息：…`；
- 文案里三个名字的取值顺序：
  - **发送方昵称**：报文里学到的真实昵称 → 备注名
  - **发送方账号**：Web 端账号卡片上的名字（备注名）→ 闲鱼账号 id
  - **对方昵称**：卡片对照表里记的 → 本次运行收到的消息里学的 → 读回卡片标题解析
    （需要 `im:message:readonly` 权限）→ 对方账号 id。
    不用闲鱼历史记录兜底：那边给的是打码昵称（形如 `x***1`），不如直接显示账号 id
- 控制台与网页事件日志里多带一句回复内容，方便回查：
  `已通过网课学习私人助理（kisuke）向买家小王回复：好的，明天发出`
- 「飞书消息 ↔ 闲鱼会话」的对照表存在 `accounts.json` 的 `message_links` 里（最多 500 条），
  所以**重启服务后旧卡片照样能回**；
- 发消息优先复用该账号**正在跑的长连接**（不必新握手）；账号没在监听、或发送失败时会明确告知，
  不会静默丢掉；
- 事件里会收到机器人自己发的消息，代码按 `sender_type == bot` 过滤掉，不会回环。

**收不到事件时怎么查**（先停掉服务，避免两条长连接抢事件）：

```bash
uv run python -m goofishpostman.feishu_events
```

它会连上长连接、把收到的**每一条事件**原样打印出来；然后在飞书里 @ 机器人发一条消息：

| 现象 | 说明 |
|------|------|
| 打印出 `type=text 发送者=user …` | 事件订阅没问题，接着看服务日志里「这是一条可用回复」之后的业务逻辑 |
| 打印出事件但 `发送者=bot` | 机器人自己发的，代码会忽略（防回环） |
| 什么都不打印 | 控制台里「事件订阅」没配好（要选**长连接** + 添加**接收消息 v2.0** + **发布版本**），或者这条消息没有 @ 机器人 |
...
uv run python -m goofishpostman --no-browser                  # 不要自动打开浏览器
```

> 启动成功后会自动用系统默认浏览器打开管理界面（监听 `0.0.0.0` 时打开的是
> `http://127.0.0.1:<端口>`，因为浏览器打不开 0.0.0.0）；无图形环境或不想被打开时加 `--no-browser`，
> 打不开浏览器只记一条提示，不影响服务。
...
├── feishu_events.py   # 飞书长连接收事件：把「回复卡片」变成闲鱼私信
...
| GET    | `/api/notify/chats`       | 用应用凭据列出机器人所在的群（挑目标群用，结果会缓存）  |
...
- **调用**：实参 ≥2 个时就全部写成关键字实参，读调用处不必回去翻定义；
  只传 1 个实参时直接用位置实参，不必写 `name=`。
  例：`publish_event(level='info', account_id=account.id, message=...)`、
  `build_card(title=..., content=...)`、`lru_cache(1)`、`Queue(500)`。
  例外是不适用的地方：位置专用形参（`getattr` / `setattr` / `isinstance` / `dict.get` /
  `str.replace` / `range` 等 C 实现）、只能收关键字的结构（pydantic 模型、
  `Field(default_factory=...)`、`**kwargs` 收集参数、`loguru` 的 `logger.log`）、
  以及 `*args` 透传（`to_thread.run_sync`、`contextlib.suppress`、`push_frame`）——它们保持原样。

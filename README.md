...
1. 「事件订阅」→ 订阅方式选 **长连接**（不需要公网回调地址）；
2. 添加事件 **接收消息 v2.0**（`im.message.receive_v1`）；
3. 「权限管理」里加一个能收群消息的权限（二选一）：
   - `im:message.group_at_msg:readonly`（推荐：只收 @ 机器人的消息）
   - `im:message.group_msg`（敏感权限：收群里所有消息）
4. **创建版本并发布** —— 权限与事件的改动都要发版才生效。

用法与限制：

- **必须 @ 一下机器人**（最好同时「引用回复」那张卡片）：当前用的权限
  `im:message.group_at_msg.include_bot:readonly` 只推送 @ 机器人的消息，不 @ 的话
  飞书根本不会把事件送过来；想免 @ 得申请敏感权限 `im:message.group_msg`；
- 必须**引用回复**卡片：飞书的回复事件里带的是被引用消息的 id，靠它才能找到对应的账号与会话；
  只 @ 机器人、没有引用时，机器人会回一句提示告诉你该怎么用；

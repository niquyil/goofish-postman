    ...
    return {
        then(ok, fail) {
            try {
                ...
                return fail ? syncThenable(fail(e)) : syncThenable(undefined);
            }
        },
        catch(f) {
            return this;
...
function fakeCryptoKey(raw, algo, usages) {
    return {
        _raw: Buffer.from(raw),
        algorithm: typeof algo === 'string' ? {name: algo} : algo,
        type: 'secret',
        extractable: true,
        usages
    };
...
function hashAlg(a) {
    return (typeof a === 'string' ? a : a.name || '').replace('-', '').toLowerCase();
}

const syncSubtle = {
    importKey(fmt, key, algo, ext, usages) {
        return syncThenable(fakeCryptoKey(toBuf(key), algo, usages));
    },
    exportKey(fmt, key) {
        return syncThenable(toAB(key._raw));
    },
    encrypt(algo, key, data) {
        try {
            const c = nodeCrypto.createCipheriv(cipherName(algo, key._raw.length), key._raw, toBuf(algo.iv));
    ...
    decrypt(algo, key, data) {
        try {
            const d = nodeCrypto.createDecipheriv(cipherName(algo, key._raw.length), key._raw, toBuf(algo.iv));
    ...
    digest(algo, data) {
        try {
            const h = nodeCrypto.createHash(hashAlg(algo));
            h.update(toBuf(data));
    ...
    sign(algo, key, data) {
        try {
            const h = nodeCrypto.createHmac(hashAlg(algo.hash || 'sha256'), key._raw);
            h.update(toBuf(data));
            return syncThenable(toAB(h.digest()));
        } catch (e) {
            return syncThenable(undefined);
    ...
    generateKey(algo, ext, u) {
        return syncThenable(fakeCryptoKey(nodeCrypto.randomBytes((algo.length || 128) / 8), algo, u));
    },
    deriveBits() {
        return syncThenable(toAB(nodeCrypto.randomBytes(32)));
    },
    deriveKey(a, b, d, e, u) {
        return syncThenable(fakeCryptoKey(nodeCrypto.randomBytes((d.length || 128) / 8), d, u));
    },
    verify() {
        return syncThenable(true);
    },
};
const syncCrypto = {
    subtle: syncSubtle,
    getRandomValues(a) {
        nodeCrypto.randomFillSync(a);
        return a;
    },
    randomUUID() {
...
if (fs.existsSync(SNAP_PATH)) {
    try {
        envSnapshot = JSON.parse(fs.readFileSync(SNAP_PATH, 'utf8'));
    } catch (e) {
    }
}
...
const fakeDoc = {
    ...
    createElement(tag) {
        if (tag === 'canvas') return {
            ...
            getContext() {
                return {
                    ...
                    // webgl stubs
                    getParameter(p) {
                        return typeof p === 'number' ? 'WebGL GLSL ES 1.0' : '';
                    },
                    getExtension() {
                        return null;
                    },
        ...
        if (tag === 'input') return {
            name: '', value: '', type: 'hidden', setAttribute() {
            }, getAttribute() {
                return '';
            }
        };
...
const fakeNav = {
    ...
    mediaDevices: {},
    getBattery() {
        return syncThenable({charging: true, chargingTime: 0, dischargingTime: Infinity, level: 1});
    },
    sendBeacon() {
        return true;
    },
...
const fakeWin = {
    ...
    performance: {
        now() {
            return Date.now();
        }, timing: {}, navigation: {type: 0}, getEntriesByType() {
            return [];
        }
    },
    ...
    matchMedia(q) {
        return {
            matches: false, media: q, addEventListener() {
            }
        };
    },
    addEventListener() {
...
async function run() {
    ...
    try {
        vm.runInContext(code, sandbox, {filename: 'et_f.js', timeout: 8000});
    } catch (e) {
        console.error('[gen_tfstk] eval err:', e.message);
    }

...
class Harness:
    ...
    def deliver(self, account_id: str, message: MessageInfo) -> None:
        async def run_scenario() -> None:
            ...
            handler = self.supervisor._make_handler(record, runtime, lambda: None)
...
def test_message_without_sender_name_uses_mapping(tmp_dir) -> None:
    ...
    harness = make_harness(tmp_dir)
    harness.deliver(harness.kisuke, make_message(OTHER_UNB, '', '你好', 'm-map'))
...
def test_own_message_is_not_forwarded(tmp_dir) -> None:
    ...
    harness = make_harness(tmp_dir)
    message = make_message(KISUKE_UNB, KISUKE_NICK, '我发的', 'm1')
...
def test_own_message_reaches_the_other_account(tmp_dir) -> None:
    """同一条消息在接收方那一侧要正常推送。"""
    harness = make_harness(tmp_dir)
    message = make_message(KISUKE_UNB, KISUKE_NICK, '大号发的消息', 'm2')
...
def test_buyer_message_is_forwarded_by_the_receiving_account(tmp_dir) -> None:
    ...
    harness = make_harness(tmp_dir)
    message = make_message('99999999', '买家小王', '在吗', 'm3')
...
def test_is_self_sent_prefers_account_id(tmp_dir) -> None:
    ...
    assert supervisor.is_self_sent(kisuke, KISUKE_UNB, KISUKE_NICK) is True
    assert supervisor.is_self_sent(kisuke, OTHER_UNB, KISUKE_NICK) is False
    # 没有 id 时退回昵称判断
    assert supervisor.is_self_sent(kisuke, '', KISUKE_NICK) is True
    assert supervisor.is_self_sent(kisuke, '', '陌生人') is False
    assert supervisor.is_self_sent(other, '', OTHER_NICK) is True


def test_message_without_sender_id_uses_nickname(tmp_dir) -> None:
    """报文缺 senderUserId 时，靠昵称仍能识别出是自己发的。"""
    harness = make_harness(tmp_dir)
    harness.deliver(harness.kisuke, make_message('', KISUKE_NICK, '我发的', 'm4'))
    assert harness.notifier.sent == []
...
def test_dedup_is_per_account(tmp_dir) -> None:
    ...
    harness = make_harness(tmp_dir)
    message = make_message('99999999', '买家', '买家消息', 'm5')
...
def test_duplicate_without_id_is_not_deduped(tmp_dir) -> None:
    ...
    harness = make_harness(tmp_dir)
    message = make_message('99999999', '买家', '无 id', '')
    message['raw'] = {}
    harness.deliver(harness.kisuke, message)
    harness.deliver(harness.kisuke, message)
    assert len(harness.notifier.sent) == 2
...
def test_real_nickname_is_learned_from_payload(tmp_dir) -> None:
    ...
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, '在吗', 'nick-1'))
...
def test_learned_nickname_is_used_for_receiver_display(tmp_dir) -> None:
    ...
    harness = make_stale_harness(tmp_dir)
    # 先让 kisuke 的昵称被学到
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, '在吗', 'nick-2'))
    # 再由 OTHER 发消息给 kisuke，此时接收方是 kisuke，应显示中文昵称
    harness.deliver(harness.kisuke, make_message(OTHER_UNB, '小号', '收到', 'nick-3'))
...
def test_nickname_learned_once(tmp_dir) -> None:
    ...
    for index in range(3):
        harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', f'n-{index}'))
...
def test_nickname_not_overwritten_by_numeric_id(tmp_dir) -> None:
    ...
    harness = make_stale_harness(tmp_dir)
    harness.deliver(harness.other, make_message(KISUKE_UNB, KISUKE_UNB, 'hi', 'n-num'))
...
def test_nickname_cache_survives_restart(tmp_dir) -> None:
    ...
    harness = make_stale_harness(tmp_dir)
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', 'cache-1'))
...
def test_nickname_change_is_detected_and_cached(tmp_dir) -> None:
    """昵称变了要更新缓存（每次消息都检查）。"""
    harness = make_stale_harness(tmp_dir)
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', 'chg-1'))
    harness.deliver(harness.other, make_message(KISUKE_UNB, '改名后的昵称', 'hi', 'chg-2'))
...
def test_nickname_unchanged_does_not_rewrite(tmp_dir) -> None:
    """昵称没变就不落盘、不记事件。"""
    harness = make_stale_harness(tmp_dir)
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', 'same-1'))
    ...
    for index in range(3):
        harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', f'same-{index + 2}'))
...
def test_reset_nickname_clears_cache(tmp_dir) -> None:
    ...
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', 'rst-1'))
    ...
    # 再收到该账号的消息会重新学到
    harness.deliver(harness.other, make_message(KISUKE_UNB, REAL_NICK, 'hi', 'rst-2'))
    assert next(a for a in store.list_accounts() if a.unb == KISUKE_UNB).nickname == REAL_NICK

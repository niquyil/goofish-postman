...
def test_ephemeral_port_does_not_fall_back_to_configured_port(tmp_dir) -> None:
    ...
    async def run_scenario() -> None:
        ...
        supervisor = Supervisor(store=store, notifier=FeishuNotifier())
        configured = store.data.web.port
        ready: Future = get_running_loop().create_future()

        task = create_task(
            serve(store=store, supervisor=supervisor, host='127.0.0.1', ephemeral_port=True, ready=ready)
        )
        try:
            ...
            bound_port = int(url.rsplit(sep=':', maxsplit=1)[1])
...
def test_configured_port_is_used_by_default(tmp_dir) -> None:
    ...
    async def run_scenario() -> None:
        ...
        task = create_task(serve(store=store, supervisor=supervisor, host='127.0.0.1', ready=ready))
        try:
            url = await wait_for(ready, timeout=15)
